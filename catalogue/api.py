...
from .models import Application, Record, Style
...
class OwsResourceSerializer(serializers.Serializer):
    ...
    def run_validation(self, data=empty):
        ...
        if (
            (data.get("wfs", False) and (not data["wfs_endpoint"] or not data["wfs_version"]))
            or (data.get("wms", False) and (not data["wms_endpoint"] or not data["wms_version"]))
            or (data.get("gwc", False) and not data["gwc_endpoint"])
        ):
    ...
    def get_links(self, record, validated_data):
        ...
        if validated_data["gwc"]:
            ...
            for endpoint in gwc_endpoints:
                links.append(record.generate_ows_link(endpoint, "GWC", validated_data["wms_version"]))
        elif validated_data["wms"]:
            links.append(record.generate_ows_link(validated_data["wms_endpoint"], "WMS", validated_data["wms_version"]))
        if validated_data["wfs"]:
            links.append(record.generate_ows_link(validated_data["wfs_endpoint"], "WFS", validated_data["wfs_version"]))
    ...
    def save(self, **kwargs):
        ...
        if source_legend:
            ...
            tmpRecord.source_legend.save(
                "upload{}".format(source_legend.get("ext", "")), ContentFile(base64.b64decode(source_legend["content"])), save=False
            )
        ...
        for uploaded_style in styles_data:
            ...
            try:
                styleSerializer.instance = Style.objects.get(
                    record=self.instance, format=uploaded_style["format"].upper(), name=uploaded_style["name"]
                )
